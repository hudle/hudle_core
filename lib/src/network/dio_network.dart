...
class ErrorCode {
  ...
  static const COULD_NOT_CONNECT = 1001;
  static const TIME_OUT = 1002;
...
abstract class DioNetwork {
  ...
  DioNetwork() {
     _dio = Dio(BaseOptions(baseUrl: injectBaseUrl() , headers: injectHeaders()))
      ..interceptors.add(
        PrettyDioLogger(
            requestBody: kDebugMode, requestHeader: kDebugMode, responseBody: kDebugMode),
      );
  }
...
BaseResponse<T> _handleDioError<T>(DioError e) {
  switch (e.type) {
    ...
    case DioErrorType.response:
      switch (e.response?.statusCode) {
        ...
        case 422:
          ...
          final validationError = ValidationError.fromJson(e.response?.data['errors']);
    ...
    case DioErrorType.connectTimeout:
      return BaseResponse.fromError("Time out", errorCode: ErrorCode.TIME_OUT);
  }
}
